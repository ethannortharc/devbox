...
If you want the VM to write directly to your host filesystem (the traditional shared-folder behavior), choose it at creation or switch an existing Lima/Incus box to the current project:

```bash
# Choose writable mode when creating the box
devbox create --name my-vm --writable

# Or switch an existing box to this directory in writable mode
devbox use my-vm --writable
