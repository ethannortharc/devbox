...
mod tests {
    ...
    /// Blank out parenthesised sub-expressions and `''…''` blocks.
    ///
    ...
    /// bare attribute names at the top level of the list are what the catalog
    /// mirrors, so everything else is removed before comparing.
    fn bare_list_only(body: &str) -> String {
    ...
    fn set_index_imports_every_set() {
        let default = set_file("default").expect("default.nix is embedded");
        for set in NIX_SETS {
            assert!(
                default.contains(&format!("./{}.nix", set.name)),
                "nix/sets/default.nix does not import the '{}' set, so selecting it \
